...
int getHeight(TreeNode root) {
    ...
    res = Math.max(res, leftHeight + rightHeight + 1);// res不断记录左右子节点高度 + 自己
    return Math.max(leftHeight, rightHeight) + 1;// 返回该节点的高度 == 该节点子节点高度 + 自己
}
```
...
[hot100 230. 二叉搜索树中第 K 小的元素](https://leetcode.cn/problems/kth-smallest-element-in-a-bst/)
