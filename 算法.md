...
[hot100 25. K 个一组翻转链表](https://leetcode.cn/problems/reverse-nodes-in-k-group/)
...
public int firstMissingPositive(int[] nums) {
    //只能使用O(1)的空间复杂度
    //数组中的数组若出现负数或出现 ≥ len+1 的数，代表nums中肯定有一个正数缺失
...
4. 动态规划：
   1. 01背包和完全背包：
      - 遍历顺序：除排列问题：先遍历物品，再遍历容量（组合遍历容量是倒序）
      - 求最大value不需要初始化
   ...
   3. 回文子串问题：
      ...
      - 有的题型dp是boolean类型，有的提醒dp是int类型表示最长长度
